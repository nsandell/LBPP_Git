...
class SparseCPTCreator implements ICPDCreator
{
	...
	public ParserFunction parseLine(String[] args, PrintStream st) throws ParserException
	{
		try {
	...
	public String description(){return "Create a conditional probability table that is stored and computed in a sparse manner.  " +
			"Takes a list of in-line arguments.  The first argument is the cardinality" + 
	...
	private static Pattern patt = Pattern.compile("^\\s*((\\d+\\s*)+)(\\d+)\\s+([\\.e\\-0-9E]+)$");
}