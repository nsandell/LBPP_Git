...
public class CountdownNode3 extends DBNNode implements ICountdownNode {
	...
	public CountdownNode3(DynamicBayesianNetwork net, String name, int maxlen)
	{
		super(net,name);
	...
	private double betheFreeEnergy(int t) throws BNException {
		...
		// Adjust E, H1, and H2 as if there was a discrete node cutting from truncation size down to 2.
		if(cutDist == null)
		{
			double[][] dist = new double[this.truncation][2];
			dist[0][0] = 1;
			for(int i = 1; i < this.truncation; i++)
				dist[i][1] = 1;
			this.cutDist = new DiscreteCPT(dist,2);
		}

		double adju = 0;
		for(int i = 0; i < this.intraChildren.size(); i++)
		{
			FiniteDiscreteMessage localLambda = this.childrenMessages.getIncomingLambdas(t).get(i);
			FiniteDiscreteMessage localPi = this.childrenMessages.getOutgoingPis(t).get(i);
			FiniteDiscreteMessage incPi;
			if(t < this.bayesNet.T-1)
			{
				incPi = this.pis.get(t).multiply(this.lambdas.get(t));
				incPi.normalize();
			}
			else
				incPi = this.pis.get(t).copy();
			
			for(int j = 0; j < this.intraChildren.size(); j++)
			{
				if(i==j)
					continue;
				incPi.setValue(0, incPi.getValue(0)*this.childrenMessages.getIncomingLambdas(t).get(j).getValue(0));
				for(int k = 1; k < this.truncation; k++)
					incPi.setValue(k, incPi.getValue(k)*this.childrenMessages.getIncomingLambdas(t).get(j).getValue(1));
				incPi.normalize();
			}
			
			DynamicMessageSet<FiniteDiscreteMessage> incoming_pis = new DynamicMessageSet<FiniteDiscreteMessage>();
			incoming_pis.addIntraMessage(incPi);
			FiniteDiscreteMessage marginalss = localPi.multiply(localLambda);
			marginalss.normalize();
			adju += this.cutDist.computeBethePotential(incoming_pis, localLambda, marginalss, null, 1);
		}
		double ret = E + H1 - H2 + adju;
		return ret;
	}
	private DiscreteCPT cutDist = null;
