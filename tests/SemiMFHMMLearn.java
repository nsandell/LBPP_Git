...
import complex.featural.controllers.FHSMMX;
import complex.featural.controllers.MFHSMMController;
import complex.featural.controllers.MFHSMMController.MFHSMMInitialParamGenerator;
...
public class SemiMFHMMLearn {
	
	
	public static class ParamGen implements MFHSMMInitialParamGenerator
	{
		@Override
		public DiscreteCPT getInitialA() {
			try
			{
				//new double[][]{{0,1},{1,0},{.9,.1},{.1,.9}})); //TRUTH
				return new DiscreteCPT(new int[]{2,2}, 2, new double[][]{{0,1},{1,0},{.9,.1},{.1,.9}});
//				return new DiscreteCPT(new int[]{2,2}, 2, new double[][]{{0,1},{1,0},{1,0},{0,1}});
		...
		public SwitchingCountdownDistribution getInitialQ() {
			...
			dist.distributions = new PoissonCountdown[2];
			dist.distributions[0]  = new PoissonCountdown(50,5);
			dist.distributions[1]  = new PoissonCountdown(50,25);
			return dist;
		}
	...
	public static class YSemiORWrapper implements IFeaturalChild
	{
		public YSemiORWrapper(String name,int[] o, IDynamicBayesNet net) throws BNException
		{
			...
			prior.exp_tr[0][0] = 4; prior.exp_tr[0][1] = 1;
	...
	public static void main(String[] args) throws Exception
	{
		...
		MFHSMMController cont = new MFHSMMController(net,children,new ParamGen(),2);
		Vector<ProposalGenerator<IFeaturalChild, FHSMMX>> gens = new Vector<ProposalGenerator<IFeaturalChild,FHSMMX>>();
		gens.add(new CoherenceSplitter<IFeaturalChild,FHSMMX>(1,1,1,1));
		gens.add(new SimilarityMerger<IFeaturalChild,FHSMMX>(1,1,1,1));
		gens.add(new CoherenceAdder<IFeaturalChild,FHSMMX>(1,1,1,1));
		gens.add(new CoherenceUniqueParenter<IFeaturalChild, FHSMMX>(1, 1, 1, 1));
		IBPMixture<IFeaturalChild,FHSMMX> mix = new IBPMixture<IFeaturalChild,FHSMMX>(gens,
				new double[]{.25,.25,.25,.25},
				new double[]{.15, .15, .7});
		cont.setLogger(System.out);

		IBPMModelOptions<IFeaturalChild,FHSMMX> opts = new IBPMModelOptions<IFeaturalChild,FHSMMX>(cont, ass);
